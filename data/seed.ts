import type { Shop, Transaction } from "@/lib/types";

const now = "2026-06-06T00:00:00.000Z";

export const seedShops: Shop[] = [
  {
    id: "shop-lanaartsco",
    name: "LanaArtsCo",
    active: true,
    feeCalculationMode: "automatic",
    estimatedFeePercentage: 11,
    estimatedFixedFee: 0.3,
    createdAt: now,
    updatedAt: now
  },
  {
    id: "shop-luxuriumarts",
    name: "LuxuriumArts",
    active: true,
    feeCalculationMode: "automatic",
    estimatedFeePercentage: 11,
    estimatedFixedFee: 0.3,
    createdAt: now,
    updatedAt: now
  },
  {
    id: "shop-oaklivings",
    name: "OakLivings",
    active: true,
    feeCalculationMode: "automatic",
    estimatedFeePercentage: 11,
    estimatedFixedFee: 0.3,
    createdAt: now,
    updatedAt: now
  },
  {
    id: "shop-sequoiainteriors",
    name: "SequoiaInteriors",
    active: true,
    feeCalculationMode: "automatic",
    estimatedFeePercentage: 11,
    estimatedFixedFee: 0.3,
    createdAt: now,
    updatedAt: now
  }
];

export const seedTransactions: Transaction[] = [
  {
    id: "txn-4079480146",
    shopId: "shop-sequoiainteriors",
    date: "2026-06-06",
    month: "2026-06-01",
    orderNumber: "4079480146",
    status: "paid",
    grossRevenue: 255.32,
    refunds: 0,
    etsyFees: 33.97,
    etsyAds: 0,
    productCost: 83,
    shippingPaid: 0,
    otherFees: 0,
    netRevenue: 221.35,
    netProfit: 138.35,
    margin: 0.625,
    notes: "",
    estimatedEtsyFees: 33.97,
    actualEtsyFees: null,
    feesStatus: "estimated",
    refundType: null,
    refundAmount: 0,
    refundedAt: null,
    productCostRecovered: false,
    etsyFeesRefunded: 0,
    createdAt: now,
    updatedAt: now
  },
  {
    id: "txn-4082071229",
    shopId: "shop-sequoiainteriors",
    date: "2026-06-04",
    month: "2026-06-01",
    orderNumber: "4082071229",
    status: "paid",
    grossRevenue: 229.59,
    refunds: 0,
    etsyFees: 30.58,
    etsyAds: 0,
    productCost: 80.39,
    shippingPaid: 0,
    otherFees: 0,
    netRevenue: 199.01,
    netProfit: 118.62,
    margin: 0.5961,
    notes: "",
    estimatedEtsyFees: 30.58,
    actualEtsyFees: null,
    feesStatus: "estimated",
    refundType: null,
    refundAmount: 0,
    refundedAt: null,
    productCostRecovered: false,
    etsyFeesRefunded: 0,
    createdAt: now,
    updatedAt: now
  },
  {
    id: "txn-4083802925",
    shopId: "shop-sequoiainteriors",
    date: "2026-06-06",
    month: "2026-06-01",
    orderNumber: "4083802925",
    status: "paid",
    grossRevenue: 234.6,
    refunds: 0,
    etsyFees: 31.24,
    etsyAds: 0,
    productCost: 60,
    shippingPaid: 0,
    otherFees: 0,
    netRevenue: 203.36,
    netProfit: 143.36,
    margin: 0.705,
    notes: "",
    estimatedEtsyFees: 31.24,
    actualEtsyFees: null,
    feesStatus: "estimated",
    refundType: null,
    refundAmount: 0,
    refundedAt: null,
    productCostRecovered: false,
    etsyFeesRefunded: 0,
    createdAt: now,
    updatedAt: now
  },
  {
    id: "txn-4083437509",
    shopId: "shop-oaklivings",
    date: "2026-06-06",
    month: "2026-06-01",
    orderNumber: "4083437509",
    status: "paid",
    grossRevenue: 216.76,
    refunds: 0,
    etsyFees: 28.89,
    etsyAds: 0,
    productCost: 53,
    shippingPaid: 0,
    otherFees: 0,
    netRevenue: 187.87,
    netProfit: 134.87,
    margin: 0.7179,
    notes: "",
    estimatedEtsyFees: 28.89,
    actualEtsyFees: null,
    feesStatus: "estimated",
    refundType: null,
    refundAmount: 0,
    refundedAt: null,
    productCostRecovered: false,
    etsyFeesRefunded: 0,
    createdAt: now,
    updatedAt: now
  },
  {
    id: "txn-luxuriumarts-20260601",
    shopId: "shop-luxuriumarts",
    date: "2026-06-01",
    month: "2026-06-01",
    orderNumber: "",
    status: "paid",
    grossRevenue: 183.24,
    refunds: 0,
    etsyFees: 30.79,
    etsyAds: 0,
    productCost: 83,
    shippingPaid: 0,
    otherFees: 0,
    netRevenue: 152.45,
    netProfit: 69.45,
    margin: 0.4556,
    notes: "",
    estimatedEtsyFees: 30.79,
    actualEtsyFees: null,
    feesStatus: "estimated",
    refundType: null,
    refundAmount: 0,
    refundedAt: null,
    productCostRecovered: false,
    etsyFeesRefunded: 0,
    createdAt: now,
    updatedAt: now
  },
  {
    id: "txn-lanaartsco-20260601",
    shopId: "shop-lanaartsco",
    date: "2026-06-01",
    month: "2026-06-01",
    orderNumber: "",
    status: "paid",
    grossRevenue: 570.61,
    refunds: 0,
    etsyFees: 82.47,
    etsyAds: 0,
    productCost: 259,
    shippingPaid: 0,
    otherFees: 0,
    netRevenue: 488.14,
    netProfit: 229.14,
    margin: 0.4694,
    notes: "",
    estimatedEtsyFees: 82.47,
    actualEtsyFees: null,
    feesStatus: "estimated",
    refundType: null,
    refundAmount: 0,
    refundedAt: null,
    productCostRecovered: false,
    etsyFeesRefunded: 0,
    createdAt: now,
    updatedAt: now
  }
];
